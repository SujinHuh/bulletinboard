...
class TodoApi {
    ...
    eventBind() {
        this.prop.el.id.todoInputBt.addEventListener('click', this.addTodo.bind(this));
    }
    ...
    listEventBind() {
        ...
        btArray.forEach((value, index) => {
            value.addEventListener('click', this.modify.bind(this));
        });
    }
        ...
        axios.post('/todo/add', {text: this.prop.el.id.todoInput.value})
            .then((res) => {
                if (res.data.code === '0000') {
                    ...
                    this.prop.el.class.todoBt[length].addEventListener('click', this.modify.bind(this));
                } else {
                    alert('추가 실패.');
                }
            })
